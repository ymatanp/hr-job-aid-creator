...
5. **Preserve the slide background.** The screencast slides carry a slide-level background override (`<p:bg>` in `p:cSld`, a white `<a:schemeClr val="bg2"/>`) — the "Segue Midnight" *layout* background is dark, so a slide that loses this override renders dark and its dark body text becomes invisible. When duplicating, ensure each copy keeps its own `<p:bg>`; if a copy is missing it, re-insert the `<p:bg>` (as the first child of `p:cSld`) copied from the source slide.
6. **Verify contrast by rendering.** After building, render the procedural slides to images (e.g. LibreOffice `soffice --headless --convert-to pdf`, then rasterize) and confirm the background is white and the title/step captions (dark navy `01182E`/`07182D`) are clearly readable — not dark-on-dark. Do not "fix" this by turning text white; restore the white background instead, so the deck matches the template design.
